...
describe("User routes", () => {
	...
	it("POST /me/upload-url - should return signed upload URL", async () => {
	...
	it("POST /me/upload-pic - should update user profilePic key", async () => {
		// Create user
		const user = await UserModel.create({
	...
	it("GET /me/profile-pic - should return signed URL for profile pic", async () => {
		...
		const res = await request(app).get("/me/profile-pic?key=somekey");
		expect(res.status).toBe(200);
